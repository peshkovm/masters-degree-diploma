...
public class PerformanceTest {
  ...
  private static final String RES_FILE_PATH =
      "src/main/resources/main/operationbased/lwwregister/PerformanceTest.csv";
  ...
  public static class GCounterCmrdtState extends CrdtState {
    protected final String crdtId = "totalPrice";
    protected Vector<LWWRegisterCmRDT> lwwRegisters;
    ...
    public void check(BenchmarkParams params) throws InterruptedException {
      lwwRegisters.forEach(
          register -> {
            if (register.query() != TIMES_TO_INCREMENT) {
              throw new AssertionError(
                  "\nExpected :" + TIMES_TO_INCREMENT + "\nActual   :" + register.query());
            }
          });

      deleteResource(crdtId, ResourceType.LWWRegisterCmRDT);
    }
  }
  ...
  public static class GCounterTest {
    ...
    public void increment(final GCounterCmrdtState state, final Blackhole bh)
      ...
      final LWWRegisterCmRDT sourceGCounter = state.lwwRegisters.get(0);
      sourceGCounter.assign(sourceGCounter.value() + 1L);

      if (sourceGCounter.query() == TIMES_TO_INCREMENT) {
        for (int i = 0; i < NUM_OF_SECONDS_TO_WAIT / 100; i++) {
          if (!state.lwwRegisters.forAll(register -> register.query() == TIMES_TO_INCREMENT)) {
